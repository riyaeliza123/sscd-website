...
def transform_targets(y_train, anchors, anchor_masks, height, width):
...
def transform_images(x_train, height, width):
...
def parse_tfrecord(tfrecord, class_table, height, width):
...
def load_tfrecord_dataset(file_pattern, class_file, height, width):      
    ...
    return dataset.map(lambda x: parse_tfrecord(x, class_table, height, width))
